...
class GomokuAI:
    ...
    def select_best_move(self, board: List[List[int]], my_color: int, 
            ...
            """
            Selects the best move using a score-based evaluation strategy.

            Args:
                board: 15x15 board, 0=empty, 1=black, 2=white
                my_color: Our color (1 or 2)
                opponent_color: Opponent's color (1 or 2)

            Returns:
                (row, col): The chosen move
            """
            # 1. Opening move
            ...
            def _evaluate_line(r, c, dr, dc, color):
                """Evaluates a single line for scoring."""
                count1 = 0
                empty_ends1 = 0
                x, y = r + dr, c + dc
                while 0 <= x < self.BOARD_SIZE and 0 <= y < self.BOARD_SIZE and board[x][y] == color:
                    count1 += 1
                    x += dr
                    y += dc
                if 0 <= x < self.BOARD_SIZE and 0 <= y < self.BOARD_SIZE and board[x][y] == self.EMPTY:
                    empty_ends1 += 1
                ...
                empty_ends = empty_ends1 + empty_ends2
            ...
            def _calculate_score(r, c, color):
                """Calculates the score for placing a stone at (r, c)."""
                if board[r][c] != self.EMPTY:
                    return -1

                total_score = 0
                for dr, dc in self.DIRECTIONS:
                    total_score += _evaluate_line(r, c, dr, dc, color)
                return total_score
            ...
            for r, c in candidates:
                my_score = _calculate_score(r, c, my_color)
                opponent_score = _calculate_score(r, c, opponent_color)

                score = my_score + opponent_score * 1.1
            ...
            if best_move == (-1, -1):
                # Fallback to the first candidate if no move seems better than others
                return candidates[0]
