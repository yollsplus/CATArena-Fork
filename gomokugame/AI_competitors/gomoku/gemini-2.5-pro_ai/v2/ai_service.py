...
class GomokuAI:
    ...
    def select_best_move(self, board: List[List[int]], my_color: int, 
            ...
            """
            Selects the best move by implementing an advanced threat detection and
            evaluation strategy.

            Args:
                board: 15x15 board state.
                my_color: The AI's color.
                opponent_color: The opponent's color.

            Returns:
                The best move (row, col) to make.
            """
            # 1. Handle opening move
            if self._is_empty_board(board):
                return (self.BOARD_SIZE // 2, self.BOARD_SIZE // 2)

            # 2. Prioritize immediate win
            win_move = self._find_winning_move(board, my_color)
            if win_move:
                return win_move

            # 3. Block opponent's immediate win
            defend_move = self._find_winning_move(board, opponent_color)
            if defend_move:
                return defend_move

            # --- Nested Helper Functions for Advanced Evaluation ---

            def get_dynamic_candidates(board: List[List[int]]) -> List[Tuple[int, int]]:
                """
                Get empty positions near existing stones with a dynamic distance.
                """
                stone_count = sum(row.count(self.BLACK) + row.count(self.WHITE) for row in board)
                        ...
                        if board[r][c] != self.EMPTY:
                            for dr in range(-distance, distance + 1):
                                for dc in range(-distance, distance + 1):
                                    nr, nc = r + dr, c + dc
                                    if 0 <= nr < self.BOARD_SIZE and 0 <= nc < self.BOARD_SIZE and board[nr][nc] == self.EMPTY:
                                        candidates.add((nr, nc))
                return list(candidates)

            def score_pattern(consecutive: int, open_ends: int, is_broken: bool) -> int:
                """Scores a detected pattern."""
                if consecutive >= 5:
                    return 1000000
                if consecutive == 4:
                    return 10000 if open_ends == 2 else 500
                if is_broken and consecutive == 3: # Broken four (e.g., X_XXX)
                    return 400
                if consecutive == 3:
                    return 1000 if open_ends == 2 else 200
                if is_broken and consecutive == 2: # Broken three (e.g., X_XX)
                    return 150
                if consecutive == 2:
                    return 100 if open_ends == 2 else 10
                return 0

            def evaluate_line(line: List[int], color: int) -> int:
                """Evaluates a single line (row, col, or diagonal)."""
                score = 0
                for i in range(len(line) - 4):
                    window = line[i:i+5]
                    my_stones = window.count(color)
                    opponent_stones = window.count(opponent_color)
                    ...
                    if my_stones > 0:
                        if window[0] == self.EMPTY: open_ends += 1
                        if window[4] == self.EMPTY: open_ends += 1

                        if window == [color, color, color, color, self.EMPTY] or window == [self.EMPTY, color, color, color, color]:
                            consecutive = 4
                        elif window == [color, color, color, self.EMPTY, color] or window == [color, self.EMPTY, color, color, color]:
                            consecutive = 4; is_broken=True
                        elif window == [self.EMPTY, color, color, color, self.EMPTY]:
                            consecutive = 3; open_ends = 2

                        score += score_pattern(my_stones, open_ends, False)

                return score

            def get_position_score(r: int, c: int, color_to_eval: int) -> int:
                """Calculates the score for a single position."""
                total_score = 0
                # Temporarily make the move
                board[r][c] = color_to_eval

                # Horizontal
                row_line = board[r]
                total_score += evaluate_line(row_line, color_to_eval)

                # Vertical
                col_line = [board[i][c] for i in range(self.BOARD_SIZE)]
                total_score += evaluate_line(col_line, color_to_eval)

                # Diagonals
                diag1 = [board[r+i][c+i] for i in range(-4, 5) if 0 <= r+i < 15 and 0 <= c+i < 15]
                diag2 = [board[r+i][c-i] for i in range(-4, 5) if 0 <= r+i < 15 and 0 <= c-i < 15]
                total_score += evaluate_line(diag1, color_to_eval)
                total_score += evaluate_line(diag2, color_to_eval)
            ...
            candidates = get_dynamic_candidates(board)
            if not candidates:
            ...
            for r, c in candidates:
                my_score = get_position_score(r, c, my_color)
                opponent_score = get_position_score(r, c, opponent_color)

                # Combine scores, prioritizing defense
                score = my_score + opponent_score * 1.5

                if score > best_score:
                    best_score = score
                    best_move = (r, c)

            return best_move if best_move != (-1, -1) else candidates[0]
