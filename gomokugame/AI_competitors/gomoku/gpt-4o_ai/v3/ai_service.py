...
class GomokuAI:
    ...
    def select_best_move(self, board: List[List[int]], my_color: int, opponent_color: int) -> Tuple[int, int]:
        """
        Enhanced strategy for selecting the best move.
        Implements dynamic threat evaluation and strategic control.
        """
        # Dynamic Threat Evaluation Function
        def evaluate_threats(board, player):
            # Define patterns for potential threats
            patterns = [
                (player, player, player, 0, 0),
                (0, player, player, player, 0),
                (0, 0, player, player, player)
            ]
            # Check for threats and return positions to block or extend
            threat_positions = []
            for pattern in patterns:
                threat_positions.extend(self._find_pattern_on_board(board, pattern))
            return threat_positions

        def central_control_move(board):
            # Evaluate and choose moves that enhance central control
            central_area = [(x, y) for x in range(5, 10) for y in range(5, 10)]
            best_move = self._evaluate_positions_for_control(board, central_area)
            return best_move
        ...
        def evaluate_position(x, y):
            ...
            for dx, dy in self.DIRECTIONS:
                count_my, count_opponent = self._count_consecutive(board, x, y, my_color, dx, dy), self._count_consecutive(board, x, y, opponent_color, dx, dy)
                if count_my >= 4:
                    score += 100
                elif count_my == 3:
                    score += 10
            ...
            center_weight = max(abs(x - self.BOARD_SIZE // 2), abs(y - self.BOARD_SIZE // 2))
            score += 5 - center_weight  # prioritize center tiles
            return score
    ...
    def _find_pattern_on_board(self, board, pattern):
        """Identify specific patterns on the board."""
        match_positions = []
        for x in range(self.BOARD_SIZE):
            for y in range(self.BOARD_SIZE):
                for dx, dy in self.DIRECTIONS:
                    if self._matches_pattern(board, x, y, pattern, dx, dy):
                        match_positions.append((x, y))
        return match_positions


    def _matches_pattern(self, board, x, y, pattern, dx, dy):
        """Check if there's a specific pattern from (x, y) in direction (dx, dy)."""
